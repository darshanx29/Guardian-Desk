...
def fim_add(filepath):
    try:
        # Support both Windows absolute paths and Linux paths
        path = filepath
        # If it looks like a Windows path passed via URL (e.g. C:/Windows/...)
        if len(filepath) >= 2 and filepath[1] == ':':
            path = filepath          # already absolute Windows path
        elif not filepath.startswith('/'):
            path = '/' + filepath    # Linux path
        result = fim.add_path(path)
        return jsonify(result)
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route('/api/fim/remove', methods=['POST'])
def fim_remove():
    try:
        ...
        return jsonify(fim.remove_path(path))
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def fim_rebaseline():
    try:
        ...
        if not path:
            return jsonify({"error": "path required"}), 400
        return jsonify(fim.rebaseline(path))
    except Exception as e:
        return jsonify({"error": str(e)}), 500
